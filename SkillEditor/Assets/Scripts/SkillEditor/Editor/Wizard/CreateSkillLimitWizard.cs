...
public class CreateSkillLimitWizard : ScriptableWizard {
	...
	public SkillPlayAction syncWithAction;
	...
	void OnWizardCreate(){ 
		if (moveLimit) {
			var skillEvent = SkillManager.CreateSkillAction<SkillLimit> (Selection.activeGameObject.GetComponentInParent<Skill> (), startFrame,duration);
		...
		if (attackLimit) {
			var skillEvent = SkillManager.CreateSkillAction<SkillLimit> (Selection.activeGameObject.GetComponentInParent<Skill> (), startFrame,duration);
			Selection.activeObject = skillEvent.gameObject;
			skillEvent.limitType = SkillLimitType.ATTACK;
		}
